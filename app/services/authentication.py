...
async def user_login(form_data : Annotated[OAuth2PasswordRequestForm , Depends()] , db : Session = Depends(getdb)):
    ...
    access_token = create_access_token(user.id, user.username, role.name if role else "user")
    refresh_token = create_refresh_token(user.id, user.username, role.name if role else "user")
    return {
